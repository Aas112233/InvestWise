...
const StatCard: React.FC<StatCardProps> = ({ label, value, change, isPositive = true, variant = 'light' }) => {
  ...
  return (
    <div 
      className={`relative p-8 rounded-[3.5rem] min-w-[240px] flex-1 transition-all duration-700 hover:-translate-y-3 hover:rotate-x-12 perspective-2000 group cursor-default border overflow-hidden ${cardClass}`}
    >
      {/* 3D Depth Layer - Dynamic Glow */}
      <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none bg-gradient-to-tr ${
        isBrandVariant ? 'from-white/40 to-transparent' : 
        isDarkVariant ? 'from-brand/10 to-transparent' : 'from-brand/5 to-transparent'
      }`} />
      
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-6">
          <span className={`text-[10px] font-black px-4 py-2 rounded-full uppercase tracking-[0.2em] shadow-inner transition-colors ${
            isBrandVariant ? 'bg-dark/10 text-dark/70' : 
            isDarkVariant ? 'bg-white/10 dark:bg-dark/10 text-white/60 dark:text-dark/60' : 
            'bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-gray-400'
          }`}>
            {label}
          </span>
          <div className={`flex items-center gap-1.5 text-[11px] font-black drop-shadow-sm ${
            isPositive ? (isBrandVariant ? 'text-dark/80' : 'text-emerald-500') : 'text-rose-500'
          }`}>
        ...
        <div className="mb-8 transform-gpu transition-transform group-hover:translate-z-20 group-hover:scale-105 duration-700 origin-left">
          <h3 className="text-6xl font-black tracking-tighter leading-none drop-shadow-2xl">
        ...
        <div className="h-14 w-full flex items-end gap-[6px]">
          {[45, 75, 55, 90, 60, 100, 80, 110].map((h, i) => (
            <div 
              key={i} 
              className={`flex-1 rounded-t-2xl transition-all duration-1000 delay-[${i*40}ms] group-hover:translate-y-[-8px] group-hover:shadow-2xl ${
                isBrandVariant ? 'bg-dark/20' : 
