...
const errorHandler = (err, req, res, next) => {
 ...
 // Log full error details server-side with correlation ID (never exposed to client)
 console.error(`[Error] [${correlationId}] ${req.method} ${req.url}:`, {
 message: err.message,
 name: err.name,
 code: err.code,
 stack: err.stack,
 ...
 // Append to secure log file with correlation ID
 const logPath = path.join(__dirname, '../logs/server_errors.log');
 const logEntry = `[${new Date().toISOString()}] [${correlationId}] ${req.method} ${req.url} - ${err.name}: ${err.message}\nCode: ${err.code}\nStack: ${err.stack}\nHeaders: ${JSON.stringify(req.headers)}\n\n`;
 ...
 code: safeError.code,
 // Include correlation ID for support team to look up in logs
