...
const Reports: React.FC<ReportsProps> = ({ lang }) => {
    ...
    const handleGenerateReport = async () => {
        ...
        if (activeType === 'Project Expense Audit' && !selectedProjectId) {
            showNotification('Please select a project for Project Expense Audit report', 'error');
            return;
        }

        if (activeType === 'Member Deposit History' && !selectedMemberId) {
            showNotification('Please select a member for Member Deposit History report', 'error');
            return;
        }

        if (activeType === 'Project Specific Ledger' && !selectedProjectId) {
            showNotification('Please select a project for this report', 'error');
            return;
        }

        if (activeType === 'Member Specific Ledger' && !selectedMemberId) {
            showNotification('Please select a member for this report', 'error');
            return;
        }

        if (activeType === 'Fund Specific Ledger' && !selectedFundId) {
            showNotification('Please select a fund for this report', 'error');
            return;
        }
    ...
    const reportConfigs = [
        ...
        { type: 'Earnings Ledger', category: 'Incomes', icon: <TrendingUp size={20} />, desc: t('reports.descs.earningsLedger', lang) },
    ...
    return (
        <div className="compact-screen space-y-6 animate-in fade-in duration-500">
            <Toast
                    ...
                    <div className="bg-white dark:bg-[#1A221D] p-6 rounded-[2rem] card-shadow border border-gray-100 dark:border-white/5 h-full">
                        ...
                        <div className="space-y-6">
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest px-1">{t('reports.periodSelection', lang)}</label>
                                <div className="grid grid-cols-2 gap-2 bg-gray-50 dark:bg-[#111814] p-1 rounded-xl ring-1 ring-gray-200 dark:ring-white/10">
                                    {(['Monthly', 'Quarterly', 'Yearly', 'Custom'] as PeriodType[]).map(p => (
                                        <button
                                            key={p}
                                            onClick={() => setPeriodType(p)}
                                            className={`py-2.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${periodType === p ? 'bg-dark dark:bg-brand text-white dark:text-dark shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5'}`}
                                        >
                            ...
                            <div className="space-y-3">
                                {periodType === 'Monthly' && (
                                    <MonthSelect
                                        ...
                                        years={reportYears}
                                        selectClassName="px-5 py-3.5 rounded-xl text-xs"
                                    />
                                )}
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest px-1">{t('reports.exportArch', lang)}</label>
                                <div className="flex gap-2 bg-gray-50 dark:bg-[#111814] p-1 rounded-xl ring-1 ring-gray-200 dark:ring-white/10">
                                    {(['PDF', 'Excel', 'JSON'] as ExportFormat[]).map(f => (
                                        <button
                                            key={f}
                                            onClick={() => setFormat(f)}
                                            className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${format === f ? 'bg-dark dark:bg-brand text-white dark:text-dark shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5'}`}
                                        >
                                            {f}
                                        </button>
                                    ))}
                                    ...
                                    >
                                        {filteredConfigs.map((cfg) => {
                                            const typeKey = cfg.type
                                                ...
                                                .replace(/EarningsLedger/, 'earningsLedger')
