...
const Sidebar: React.FC<SidebarProps> = ({ lang, currentUser }) => {
 ...
 {isExpanded ? (
 <h3 className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-[0.2em] mb-4 px-2">
 {t(group.groupKey, lang)}
 </h3>
 ) : (
 <div className="h-px bg-gray-200 dark:bg-white/10 mb-6 mx-2" />
 )}
 ...
 >
 <span className={`relative ${isActive ? (document.documentElement.classList.contains('dark') ? 'text-black' : 'text-[#BFF300]') : 'text-gray-500 dark:text-gray-400'}`}>
 {item.icon}
