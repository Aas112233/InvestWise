...
const Deposits: React.FC<DepositsProps> = ({ lang }) => {
    ...
    const getCurrentMonthYear = () => {
        return getCurrentMonthYearLabel(lang);
    };
    ...
    const getDateFromMonthStr = (monthStr: string) => {
        return monthYearLabelToDateInput(monthStr);
    };
    ...
    const handleOpenModal = (deposit?: Deposit) => {
        if (deposit) {
            const resolvedMemberId = (deposit as any).memberMongoId ||
                globalMembers.find(m => m.memberId === deposit.memberId || m.name === deposit.memberName)?.id ||
                deposit.memberId;
                ...
                const normalizeMonth = (input: any) => {
                    ...
                    if (typeof input === 'number') {
                        ...
                        if (!isNaN(date.getTime())) {
                            return getMonthYearLabel(date.getFullYear(), date.getMonth(), lang);
                        }
                    }

                    const str = input.toString().trim();
                    const directDate = parseYearMonthValue(str);
                    if (directDate) {
                        return getMonthYearLabel(directDate.year, directDate.monthIndex, lang);
                    }
                    ...
                    if (parts.length >= 2) {
                        ...
                        const searchPart = (p: string) => {
                            ...
                            const parsedIndex = getMonthIndex(p);
                        ...
                        if (mIdx !== -1 && /^\d{4}$/.test(yPart)) {
                            return getMonthYearLabel(Number(yPart), mIdx, lang);
                        }
                    }
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // CRITICAL: Refresh ALL data from database to update cards and list
            await fetchPaginatedDeposits(currentPage, rowsPerPage, searchQuery, searchField, sortBy, sortOrder);
            await refreshTransactions();
        } catch (err: any) {
    ...
    const bulkMonthYears = [new Date().getFullYear() - 1, new Date().getFullYear()];
        ...
        <div className="compact-screen space-y-10 animate-in fade-in duration-500">
            ...
            <div id="deposits-snapshot-target" className="space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <SummaryMetricCard
                        label={t('deposits.currentMonth', lang)}
                        value={formatCurrency(totalMonthly)}
                        note={t('members.vested', lang)}
                    />
                    <SummaryMetricCard
                        label={t('deposits.totalCapital', lang)}
                        value={formatCurrency(totalAmount)}
                        note={t('deposits.lifetime', lang)}
                        variant="dark"
                    />
                    <SummaryMetricCard
                        label={t('transactions.allTypes', lang)}
                        value={paginatedData.total}
                        note={t('deposits.records', lang)}
                        noteClassName="text-gray-400 text-xs sm:text-sm"
                    />
                </div>
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b border-gray-100 dark:border-white/5">
                                            <th className="px-4 py-4 text-left text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">{t('deposits.strategicPartner', lang)}</th>
                                                ...
                                                <td className="py-4 px-4 text-center min-w-[180px]">
                                                    <div className="relative inline-block w-full">
                                                        <MonthSelect
                                                            ...
                                                            selectClassName="bg-gray-50 dark:bg-white/5 p-4 rounded-xl font-bold text-[11px] uppercase tracking-wider ring-1 ring-gray-100 dark:ring-white/5 focus:ring-brand dark:text-white"
                                                        />
                                                    </div>
                                                </td>
