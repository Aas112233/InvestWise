...
const Analysis: React.FC<AnalysisProps> = ({ lang }) => {
  ...
  const trendData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthName = getShortMonthName(d.getMonth(), lang);
  ...
  const paymentMatrixData = useMemo(() => {
    ...
    return relevantMembers.map(m => {
      ...
      MONTH_KEYS.forEach((monthKey, idx) => {
        const key = `${monthKey}-${selectedYear}`;
  ...
  const topContributorOfMonth = useMemo(() => {
    const monthIndex = MONTH_KEYS.indexOf(selectedMonth as typeof MONTH_KEYS[number]);
    const monthDeposits = deposits.filter(d => {
      const { month: dMonth, year: dYear } = getDepositPeriod(d);
      const matchesMember = selectedMember === 'All' || d.memberId === selectedMember;
      const matchesMonth = selectedMonth === ALL_MONTHS_VALUE || dMonth === monthIndex;
    ...
    return topMember ? { ...topMember, totalContributed: maxAmount } : null;
    ...
    },
    {
      ...
      render: (project) => (
        <span className={`px-4 py-1.5 rounded-full ${project.health === 'Stable' ? 'bg-emerald-500/10 text-emerald-500' : project.health === 'At Risk' ? 'bg-amber-500/10 text-amber-500' : 'bg-red-500/10 text-red-500'}`}>
          {t(`common.${(project.health || 'Stable').toLowerCase().replace(' ', '')}`, lang)}
        </span>
      )
    },
    {
      ...
      <div className="flex items-end justify-between px-2">
        ...
        <div className="flex items-center gap-4">
          ...
          <MonthSelect
            ...
            selectClassName="bg-dark dark:bg-brand text-white dark:text-dark px-6 py-3.5 rounded-2xl font-black text-xs uppercase outline-none shadow-xl shadow-brand/10 cursor-pointer ring-0 focus:ring-0"
          />
        </div>
      </div>
          ...
          <div>
            ...
            <h2 className="text-3xl font-black text-dark dark:text-white uppercase tracking-tighter leading-none">
              {selectedMonth === ALL_MONTHS_VALUE
                ...
                : getMonthName(MONTH_KEYS.indexOf(selectedMonth as typeof MONTH_KEYS[number]), lang)} {selectedYear}
            </h2>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 bg-white dark:bg-[#1A221D] p-12 rounded-[4rem] card-shadow border border-gray-100 dark:border-white/5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                  <div className="h-[250px] w-full min-w-0" style={{ minHeight: 250, width: '100%' }}>
        ...
        {activeTab === 'Dashboard' && (
          <div className="space-y-10 animate-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              ...
              {[
                { label: t('analysis.invested', lang), value: formatCurrency(metrics.totalInvested), desc: t('analysis.capitalUsed', lang), color: 'text-brand' },
                { label: t('analysis.totalAssetValue', lang), value: formatCurrency(metrics.totalAssetValue), desc: t('analysis.currentValue', lang), color: 'text-blue-500' },
                { label: t('analysis.netGain', lang), value: formatCurrency(metrics.netProfit), desc: t('analysis.totalReturn', lang), color: metrics.netProfit >= 0 ? 'text-emerald-500' : 'text-red-500' },
                { label: t('analysis.roiLabel', lang), value: `${metrics.roi}%`, desc: t('projects.performance', lang), color: Number(metrics.roi) >= 0 ? 'text-emerald-500' : 'text-red-500' }
              ].map((m, i) => (
                <SummaryMetricCard
                  key={i}
                  label={m.label}
                  value={m.value}
                  description={m.desc}
                  icon={<Activity size={18} className={m.color} />}
                ...
                <div className="flex items-center justify-between mb-8">
                  <div>
                    ...
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-2">
                      {selectedMonth === ALL_MONTHS_VALUE ? 'YTD CAPITAL DEPLOYMENT' : t('analysis.capitalDeployment', lang)}
                    </p>
                  </div>
