...
const addDeposit = asyncHandler(async (req, res) => {
 ...
 balanceAfter: (status === 'Success' || status === 'Completed') ? (balanceBefore + depositAmount) : balanceBefore
 }], { session });
 ...
 memberId,
 { $inc: { totalContributed: depositAmount } },
 { session }
 );
 }
 ...
 // Recalculate stats asynchronously to avoid blocking response
 setImmediate(() => {
...
const editDeposit = asyncHandler(async (req, res) => {
 ...
 if (!memberId || !fundId || !amount) {
 res.status(400);
 throw new Error('Member ID, Fund ID, and Amount are required');
 }
 ...
 const oldFundId = transaction.fundId.toString();
 const oldMemberId = transaction.memberId.toString();
 const oldStatus = transaction.status;
 const isCompleted = oldStatus === 'Success' || oldStatus === 'Completed';

 // Use Number() instead of parseInt() for decimal precision
 ...
 if (isCompleted) {
 // Revert Fund using atomic operation
 ...
 // Revert Member using atomic operation
 await Member.findByIdAndUpdate(
 oldMemberId,
 { $inc: { totalContributed: -oldAmount } },
 { session }
 );
 }
 ...
 // 2. Apply New Impact
 if (isCompleted) {
 // Apply to new fund using atomic operation
 await Fund.findByIdAndUpdate(
 fundId,
 ...
 // Apply to new member using atomic operation
 await Member.findByIdAndUpdate(
 memberId,
 { $inc: { totalContributed: newAmount } },
 { session }
 );
 }
...
const approveDeposit = asyncHandler(async (req, res) => {
 ...
 // Apply Financial Impact using atomic operations
 await Fund.findByIdAndUpdate(
 transaction.fundId,
...
const addEarning = asyncHandler(async (req, res) => {
 ...
 // Update Fund using atomic operation
 await Fund.findByIdAndUpdate(
 fundId,
 ...
 // Update project using atomic operation
 await Project.findByIdAndUpdate(
 projectId,
 {
 $inc: {
 currentFundBalance: earningAmount,
 ...
 const updatedProject = await Project.findById(projectId).session(session);
 ...
 date: date || Date.now(),
 balanceBefore: projBalanceBefore,
 balanceAfter: updatedProject.currentFundBalance
 });
 ...
 const updatedFund = await Fund.findById(fundId).session(session);
 const balanceAfter = projectId
 ? (await Project.findById(projectId).session(session)).currentFundBalance
 : updatedFund.balance;
 ...
 // Add audit log for earnings (was missing before)
 await logAudit({
 req,
 user: req.user,
