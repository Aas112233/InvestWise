...
start "Backend Server" cmd /k "cd /d ""%APP_DIR%"" && set PORT=%BACKEND_PORT% && set CORS_ORIGINS=http://localhost:%FRONTEND_PORT%,http://localhost:3000,http://localhost:5173,http://localhost:%BACKEND_PORT% && node index.js"

echo [+] Starting Frontend Server (Port %FRONTEND_PORT%)...
start "Frontend Server" cmd /k "cd /d ""%APP_DIR%"" && set VITE_API_URL=%FRONTEND_API_URL% && npm run dev -- --port %FRONTEND_PORT%"
